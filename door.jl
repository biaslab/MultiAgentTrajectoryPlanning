...
begin
	...
	@pgf for k in 1:10
		push!(
			...
			Plot(
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][3,1:Int(round(1 + (nr_steps-1) * (k-1) / 9))]),
					map(x -> mean(x)[2], results.posteriors[:y][3,1:Int(round(1 + (nr_steps-1) * (k-1) / 9))]),
				)
			),
			Plot(
				{
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][4,1:Int(round(1 + (nr_steps-1) * (k-1) / 9))]),
					map(x -> mean(x)[2], results.posteriors[:y][4,1:Int(round(1 + (nr_steps-1) * (k-1) / 9))]),
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[1] .+ radius .* cos.(θ),
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[2] .+ radius .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][2,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[1] .+ radius .* cos.(θ),
					mean(results.posteriors[:y][2,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[2] .+ radius .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][3,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[1] .+ radius .* cos.(θ),
					mean(results.posteriors[:y][3,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[2] .+ radius .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][4,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[1] .+ radius .* cos.(θ),
					mean(results.posteriors[:y][4,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[2] .+ radius .* sin.(θ)
				)
			),
			Plot(
				{
			...
			[
			    ...
			    Coordinate(
					environment[1].center[1] - environment[1].width/2, 
					environment[1].center[2] - environment[1].height/2
				),
				" rectangle ",
			    Coordinate(
					environment[1].center[1] + environment[1].width/2, 
			...
			[
			    ...
			    Coordinate(
					environment[2].center[1] - environment[2].width/2, 
					environment[2].center[2] - environment[2].height/2
				),
				" rectangle ",
			    Coordinate(
					environment[2].center[1] + environment[2].width/2, 
	...
	pgfsave("exports/door.pdf", fig)
	pgfsave("exports/door.tikz", fig)

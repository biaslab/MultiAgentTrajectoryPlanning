...
@model function switching_model(nr_steps; γ=1, ΔT=1)

	# controls
	u = randomvar(2, nr_steps)

	# hidden state
	x = randomvar(2, nr_steps + 1)

	# observations
	y = randomvar(2, nr_steps)
	goals = datavar(Vector{Int64}, (2, 2))
...
results = inference(
	model 			= switching_model(nr_steps),
	data  			= ( goals = goals, ),
	...
	@pgf for k in 1:10
		push!(
			...
			Plot(
				...
				Table(
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[1] .+ radius1 .* cos.(θ),
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 9))])[2] .+ radius1 .* sin.(θ)
				)
			),
			Plot(
				{

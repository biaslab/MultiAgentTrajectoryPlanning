...
# ╔═╡ fa3ebe3b-1ef9-4c1f-9849-72e1b4f3176c
md"Save figures $(@bind save_figures CheckBox(default=false))"

# ╔═╡ c4561368-15b9-49e4-a0ff-c5a85171a054
if save_figures
	mkpath("exports")
	nothing
end
...
begin
	goals = hcat([
		# agent 1: start at (-15,5) with 0 velocity, end at (15,-5) with 0 velocity
		[
			[-15, 0, 5, 0],
			[15, 0, -5, 0]
		], 
		# agent 2: start at (-15,-5) with 0 velocity, end at (15,5) with 0 velocity
		[
			[-15, 0, -5, 0],
			[15, 0, 5, 0]
		],
...
begin
	...
	draw_circle!([goals[1,3][1], goals[1,3][3]], radius3; color="orange", label="")
	draw_circle!([goals[1,4][1], goals[1,4][3]], radius4; color="green", label="")
...
function h(y1, y2, y3, y4; r1 = radius1, r2 = radius2, r3 = radius3, r4 = radius4)
	return softmin([
		norm(y1 - y2) - r1 - r2,
		norm(y1 - y3) - r1 - r3,
		norm(y1 - y4) - r1 - r4,
...
@model function crossroads_model(nr_steps; γ=10, ΔT=1)
...
@constraints function crossroads_constraints() 
	q(d, σ2) = q(d)q(σ2)
	q(u) :: PointMass
end;
...
results = inference(
	model 			= crossroads_model(nr_steps),
	data  			= ( goals = goals, ),
	constraints 	= crossroads_constraints(),
	meta 			= crossroads_meta,
	iterations 		= nr_iterations,
	returnvars 		= KeepLast(), 
	initmarginals 	= ( 
		σ2 = repeat([PointMass(1)], nr_steps),
...
begin
	if save_figures
		animation = @animate for t in 1:nr_steps
			plot(size = (600, 600), legend=false)
			...
			draw_circle!(mean(results.posteriors[:x][3,t])[[1,3]], radius3; color="orange", label="")
			draw_circle!(mean(results.posteriors[:y][4,t]), radius4; color="green", label="")
	...
	@pgf for k in 1:8
		push!(
			...
			Plot(
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][1,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
					map(x -> mean(x)[2], results.posteriors[:y][1,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
				)
			),
			Plot(
				{
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][2,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
					map(x -> mean(x)[2], results.posteriors[:y][2,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
				)
			),
			Plot(
				{
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][3,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
					map(x -> mean(x)[2], results.posteriors[:y][3,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
				)
			),
			Plot(
				{
				...
				Table(
					map(x -> mean(x)[1], results.posteriors[:y][4,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
					map(x -> mean(x)[2], results.posteriors[:y][4,1:Int(round(1 + (nr_steps-1) * (k-1) / 7))]),
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[1] .+ radius1 .* cos.(θ),
					mean(results.posteriors[:y][1,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[2] .+ radius1 .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][2,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[1] .+ radius2 .* cos.(θ),
					mean(results.posteriors[:y][2,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[2] .+ radius2 .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][3,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[1] .+ radius1 .* cos.(θ),
					mean(results.posteriors[:y][3,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[2] .+ radius1 .* sin.(θ)
				)
			),
			Plot(
				{
				...
				Table(
					mean(results.posteriors[:y][4,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[1] .+ radius2 .* cos.(θ),
					mean(results.posteriors[:y][4,Int(round(1 + (nr_steps-1) * (k-1) / 7))])[2] .+ radius2 .* sin.(θ)
				)
			),
			Plot(
				{
			...
			Plot(
				...
				Table([goals[2,4][1]],[goals[2,4][3]])
			)
		)
	end;
	if save_figures
		pgfsave("exports/crossroads.pdf", fig)
		pgfsave("exports/crossroads.tikz", fig)
	end
	fig
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "991d5bf646c9d4f4c66050391af206eb114318fa"
